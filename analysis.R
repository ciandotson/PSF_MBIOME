...
### Separate each plant species such that it has a phyloseq object just containing Non-PSF Samples ###
## Fuzzy Bean ##
# Source Community #
fb_nbulk.ps <- subset_samples(fb_bulk.ps, Soil_Treatment == "Non-PSF Soil")
fb_nbulk.ps <- subset_taxa(fb_nbulk.ps, taxa_sums(fb_nbulk.ps) > 0)

# Rhizosphere #
fb_nrhiz.ps <- subset_samples(fb_rhiz.ps, Soil_Treatment == "Non-PSF Soil")
fb_nrhiz.ps<- subset_taxa(fb_nrhiz.ps, taxa_sums(fb_nrhiz.ps) > 0)

# Root Endosphere #
fb_nroot.ps <- subset_samples(fb_root.ps, Soil.Origin == "Non-PSF Soil")
fb_nroot.ps <- subset_taxa(fb_nroot.ps, taxa_sums(fb_nroot.ps) > 0)

## Chamaecrista ##
# Source Community #
cc_nbulk.ps <- subset_samples(cc_bulk.ps, Soil_Treatment == "Non-PSF Soil")
cc_nbulk.ps <- subset_taxa(cc_nbulk.ps, taxa_sums(cc_nbulk.ps) > 0)

# Rhizosphere #
cc_nrhiz.ps <- subset_samples(cc_rhiz.ps, Soil_Treatment == "Non-PSF Soil")
cc_nrhiz.ps<- subset_taxa(cc_nrhiz.ps, taxa_sums(cc_nrhiz.ps) > 0)

# Root Endosphere #
cc_nroot.ps <- subset_samples(cc_root.ps, Soil.Origin == "Non-PSF Soil")
cc_nroot.ps <- subset_taxa(cc_nroot.ps, taxa_sums(cc_nroot.ps) > 0)

## Desmodium ##
# Source Community #
ds_nbulk.ps <- subset_samples(ds_bulk.ps, Soil_Treatment == "Non-PSF Soil")
ds_nbulk.ps <- subset_taxa(ds_nbulk.ps, taxa_sums(ds_nbulk.ps) > 0)

# Rhizosphere #
ds_nrhiz.ps <- subset_samples(ds_rhiz.ps, Soil_Treatment == "Non-PSF Soil")
ds_nrhiz.ps<- subset_taxa(ds_nrhiz.ps, taxa_sums(ds_nrhiz.ps) > 0)

# Root Endosphere #
ds_nroot.ps <- subset_samples(ds_root.ps, Soil.Origin == "Non-PSF Soil")
ds_nroot.ps <- subset_taxa(ds_nroot.ps, taxa_sums(ds_nroot.ps) > 0)

## Hog Peanut ##
# Source Community #
hp_nbulk.ps <- subset_samples(hp_bulk.ps, Soil_Treatment == "Non-PSF Soil")
hp_nbulk.ps <- subset_taxa(hp_nbulk.ps, taxa_sums(hp_nbulk.ps) > 0)

# Rhizosphere #
hp_nrhiz.ps <- subset_samples(hp_rhiz.ps, Soil_Treatment == "Non-PSF Soil")
hp_nrhiz.ps<- subset_taxa(hp_nrhiz.ps, taxa_sums(hp_nrhiz.ps) > 0)

# Root Endosphere #
hp_nroot.ps <- subset_samples(hp_root.ps, Soil.Origin == "Non-PSF Soil")
hp_nroot.ps <- subset_taxa(hp_nroot.ps, taxa_sums(hp_nroot.ps) > 0)

## Clover ##
# Source Community #
cl_nbulk.ps <- subset_samples(cl_bulk.ps, Soil_Treatment == "Non-PSF Soil")
cl_nbulk.ps <- subset_taxa(cl_nbulk.ps, taxa_sums(cl_nbulk.ps) > 0)

# Root Endosphere #
cl_nroot.ps <- subset_samples(cl_root.ps, Soil.Origin == "Non-PSF Soil")
cl_nroot.ps <- subset_taxa(cl_nroot.ps, taxa_sums(cl_nroot.ps) > 0)

## Medicago ##
# Source Community #
md_nbulk.ps <- subset_samples(md_bulk.ps, Soil_Treatment == "Non-PSF Soil")
md_nbulk.ps <- subset_taxa(md_nbulk.ps, taxa_sums(md_nbulk.ps) > 0)

# Rhizosphere #
md_nrhiz.ps <- subset_samples(md_rhiz.ps, Soil_Treatment == "Non-PSF Soil")
md_nrhiz.ps<- subset_taxa(md_nrhiz.ps, taxa_sums(md_nrhiz.ps) > 0)

# Root Endosphere #
md_nroot.ps <- subset_samples(md_root.ps, Soil.Origin == "Non-PSF Soil")
md_nroot.ps <- subset_taxa(md_nroot.ps, taxa_sums(md_nroot.ps) > 0)

### Separate each plant species such that it has a phyloseq object just containing Common Samples ###
## Fuzzy Bean ##
# Source Community #
fb_cbulk.ps <- subset_samples(fb_bulk.ps, Soil_Treatment == "Common Soil")
fb_cbulk.ps <- subset_taxa(fb_cbulk.ps, taxa_sums(fb_cbulk.ps) > 0)

# Rhizosphere #
fb_crhiz.ps <- subset_samples(fb_rhiz.ps, Soil_Treatment == "Common Soil")
fb_crhiz.ps<- subset_taxa(fb_crhiz.ps, taxa_sums(fb_crhiz.ps) > 0)

# Root Endosphere #
fb_croot.ps <- subset_samples(fb_root.ps, Soil.Origin == "Common Soil")
fb_croot.ps <- subset_taxa(fb_croot.ps, taxa_sums(fb_croot.ps) > 0)

## Chamaecrista ##
# Source Community #
cc_cbulk.ps <- subset_samples(cc_bulk.ps, Soil_Treatment == "Common Soil")
cc_cbulk.ps <- subset_taxa(cc_cbulk.ps, taxa_sums(cc_cbulk.ps) > 0)

# Rhizosphere #
cc_crhiz.ps <- subset_samples(cc_rhiz.ps, Soil_Treatment == "Common Soil")
cc_crhiz.ps<- subset_taxa(cc_crhiz.ps, taxa_sums(cc_crhiz.ps) > 0)

# Root Endosphere #
cc_croot.ps <- subset_samples(cc_root.ps, Soil.Origin == "Common Soil")
cc_croot.ps <- subset_taxa(cc_croot.ps, taxa_sums(cc_croot.ps) > 0)

## Desmodium ##
# Source Community #
ds_cbulk.ps <- subset_samples(ds_bulk.ps, Soil_Treatment == "Common Soil")
ds_cbulk.ps <- subset_taxa(ds_cbulk.ps, taxa_sums(ds_cbulk.ps) > 0)

# Rhizosphere #
ds_crhiz.ps <- subset_samples(ds_rhiz.ps, Soil_Treatment == "Common Soil")
ds_crhiz.ps<- subset_taxa(ds_crhiz.ps, taxa_sums(ds_crhiz.ps) > 0)

# Root Endosphere #
ds_croot.ps <- subset_samples(ds_root.ps, Soil.Origin == "Common Soil")
ds_croot.ps <- subset_taxa(ds_croot.ps, taxa_sums(ds_croot.ps) > 0)

## Hog Peanut ##
# Source Community #
hp_cbulk.ps <- subset_samples(hp_bulk.ps, Soil_Treatment == "Common Soil")
hp_cbulk.ps <- subset_taxa(hp_cbulk.ps, taxa_sums(hp_cbulk.ps) > 0)

# Rhizosphere #
hp_crhiz.ps <- subset_samples(hp_rhiz.ps, Soil_Treatment == "Common Soil")
hp_crhiz.ps<- subset_taxa(hp_crhiz.ps, taxa_sums(hp_crhiz.ps) > 0)

# Root Endosphere #
hp_croot.ps <- subset_samples(hp_root.ps, Soil.Origin == "Common Soil")
hp_croot.ps <- subset_taxa(hp_croot.ps, taxa_sums(hp_croot.ps) > 0)

## Clover ##
# Source Community #
cl_cbulk.ps <- subset_samples(cl_bulk.ps, Soil_Treatment == "Common Soil")
cl_cbulk.ps <- subset_taxa(cl_cbulk.ps, taxa_sums(cl_cbulk.ps) > 0)

# Rhizosphere #
cl_crhiz.ps <- subset_samples(cl_rhiz.ps, Soil_Treatment == "Common Soil")
cl_crhiz.ps<- subset_taxa(cl_crhiz.ps, taxa_sums(cl_crhiz.ps) > 0)

# Root Endosphere #
cl_croot.ps <- subset_samples(cl_root.ps, Soil.Origin == "Common Soil")
cl_croot.ps <- subset_taxa(cl_croot.ps, taxa_sums(cl_croot.ps) > 0)

## Medicago ##
# Source Community #
md_cbulk.ps <- subset_samples(md_bulk.ps, Soil_Treatment == "Common Soil")
md_cbulk.ps <- subset_taxa(md_cbulk.ps, taxa_sums(md_cbulk.ps) > 0)

# Rhizosphere #
md_crhiz.ps <- subset_samples(md_rhiz.ps, Soil_Treatment == "Common Soil")
md_crhiz.ps<- subset_taxa(md_crhiz.ps, taxa_sums(md_crhiz.ps) > 0)

# Root Endosphere #
md_croot.ps <- subset_samples(md_root.ps, Soil.Origin == "Common Soil")
md_croot.ps <- subset_taxa(md_croot.ps, taxa_sums(md_croot.ps) > 0)

### Make the histograms for each Comp x Plant X PSF Interaction ###
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
fb_nbulk_top.ps <- aggregate_top_taxa2(fb_nbulk.ps, 19, "ASV")
fb_nbulk_top.name <- names(sort(taxa_sums(fb_nbulk_top.ps), decreasing = TRUE))
fb_nbulk_top.colr <- soil.colr[fb_nbulk_top.name,]

# Create a data.frame with all the relevant information for plotting #
fb_nbulk_top.df <- psmelt(fb_nbulk_top.ps)
fb_nbulk_top.df$ASVs <- factor(fb_nbulk_top.df$ASV, levels = fb_nbulk_top.name)
...
fb_cbulk_top.name <- names(sort(taxa_sums(fb_cbulk_top.ps), decreasing = TRUE))
fb_cbulk_top.colr <- soil.colr[fb_cbulk_top.name,]

# Create a data.frame with all the relevant information for plotting #
fb_cbulk_top.df <- psmelt(fb_cbulk_top.ps)
fb_cbulk_top.df$ASVs <- factor(fb_cbulk_top.df$ASV, levels = fb_cbulk_top.name)
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
fb_nrhiz_top.ps <- aggregate_top_taxa2(fb_nrhiz.ps, 19, "ASV")
fb_nrhiz_top.name <- names(sort(taxa_sums(fb_nrhiz_top.ps), decreasing = TRUE))
fb_nrhiz_top.colr <- soil.colr[fb_nrhiz_top.name,]

# Create a data.frame with all the relevant information for plotting #
fb_nrhiz_top.df <- psmelt(fb_nrhiz_top.ps)
fb_nrhiz_top.df$ASVs <- factor(fb_nrhiz_top.df$ASV, levels = fb_nrhiz_top.name)
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
fb_crhiz_top.ps <- aggregate_top_taxa2(fb_crhiz.ps, 19, "ASV")
fb_crhiz_top.name <- names(sort(taxa_sums(fb_crhiz_top.ps), decreasing = TRUE))
fb_crhiz_top.colr <- soil.colr[fb_crhiz_top.name,]

# Create a data.frame with all the relevant information for plotting #
fb_crhiz_top.df <- psmelt(fb_crhiz_top.ps)
fb_crhiz_top.df$ASVs <- factor(fb_crhiz_top.df$ASV, levels = fb_crhiz_top.name)
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
fb_nroot_top.ps <- aggregate_top_taxa2(fb_nroot.ps, 19, "ASV")
fb_nroot_top.name <- names(sort(taxa_sums(fb_nroot_top.ps), decreasing = TRUE))
fb_nroot_top.name <- c("Other", fb_nroot_top.name)
fb_nroot_top.colr <- root.colr[unique(fb_nroot_top.name),]

# Create a data.frame with all the relevant information for plotting #
fb_nroot_top.df <- psmelt(fb_nroot_top.ps)
fb_nroot_top.df$ASVs <- factor(fb_nroot_top.df$ASV, levels = unique(fb_nroot_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
fb_croot_top.ps <- aggregate_top_taxa2(fb_croot.ps, 19, "ASV")
fb_croot_top.name <- names(sort(taxa_sums(fb_croot_top.ps), decreasing = TRUE))
fb_croot_top.name <- c("Other", fb_croot_top.name)
fb_croot_top.colr <- root.colr[unique(fb_croot_top.name),]

# Create a data.frame with all the relevant information for plotting #
fb_croot_top.df <- psmelt(fb_croot_top.ps)
fb_croot_top.df$ASVs <- factor(fb_croot_top.df$ASV, levels = unique(fb_croot_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cc_nbulk_top.ps <- aggregate_top_taxa2(cc_nbulk.ps, 19, "ASV")
cc_nbulk_top.name <- names(sort(taxa_sums(cc_nbulk_top.ps), decreasing = TRUE))
cc_nbulk_top.colr <- soil.colr[cc_nbulk_top.name,]

# Create a data.frame with all the relevant information for plotting #
cc_nbulk_top.df <- psmelt(cc_nbulk_top.ps)
cc_nbulk_top.df$ASVs <- factor(cc_nbulk_top.df$ASV, levels = cc_nbulk_top.name)
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cc_nrhiz_top.ps <- aggregate_top_taxa2(cc_nrhiz.ps, 19, "ASV")
cc_nrhiz_top.name <- names(sort(taxa_sums(cc_nrhiz_top.ps), decreasing = TRUE))
cc_nrhiz_top.name <- c("Other", cc_nrhiz_top.name)
cc_nrhiz_top.colr <- soil.colr[unique(cc_nrhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
cc_nrhiz_top.df <- psmelt(cc_nrhiz_top.ps)
cc_nrhiz_top.df$ASVs <- factor(cc_nrhiz_top.df$ASV, levels = unique(cc_nrhiz_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cc_crhiz_top.ps <- aggregate_top_taxa2(cc_crhiz.ps, 19, "ASV")
cc_crhiz_top.name <- names(sort(taxa_sums(cc_crhiz_top.ps), decreasing = TRUE))
cc_crhiz_top.name <- c("Other", cc_crhiz_top.name)
cc_crhiz_top.colr <- soil.colr[unique(cc_crhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
cc_crhiz_top.df <- psmelt(cc_crhiz_top.ps)
cc_crhiz_top.df$ASVs <- factor(cc_crhiz_top.df$ASV, levels = unique(cc_crhiz_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cc_nroot_top.ps <- aggregate_top_taxa2(cc_nroot.ps, 19, "ASV")
cc_nroot_top.name <- names(sort(taxa_sums(cc_nroot_top.ps), decreasing = TRUE))
cc_nroot_top.name <- c("Other", cc_nroot_top.name)
cc_nroot_top.colr <- root.colr[unique(cc_nroot_top.name),]

# Create a data.frame with all the relevant information for plotting #
cc_nroot_top.df <- psmelt(cc_nroot_top.ps)
cc_nroot_top.df$ASVs <- factor(cc_nroot_top.df$ASV, levels = unique(cc_nroot_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cc_croot_top.ps <- aggregate_top_taxa2(cc_croot.ps, 19, "ASV")
cc_croot_top.name <- names(sort(taxa_sums(cc_croot_top.ps), decreasing = TRUE))
cc_croot_top.name <- c("Other", cc_croot_top.name)
cc_croot_top.colr <- root.colr[unique(cc_croot_top.name),]

# Create a data.frame with all the relevant information for plotting #
cc_croot_top.df <- psmelt(cc_croot_top.ps)
cc_croot_top.df$ASVs <- factor(cc_croot_top.df$ASV, levels = unique(cc_croot_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
ds_nbulk_top.ps <- aggregate_top_taxa2(ds_nbulk.ps, 19, "ASV")
ds_nbulk_top.name <- names(sort(taxa_sums(ds_nbulk_top.ps), decreasing = TRUE))
ds_nbulk_top.colr <- soil.colr[ds_nbulk_top.name,]

# Create a data.frame with all the relevant information for plotting #
ds_nbulk_top.df <- psmelt(ds_nbulk_top.ps)
ds_nbulk_top.df$ASVs <- factor(ds_nbulk_top.df$ASV, levels = ds_nbulk_top.name)
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
ds_nrhiz_top.ps <- aggregate_top_taxa2(ds_nrhiz.ps, 19, "ASV")
ds_nrhiz_top.name <- names(sort(taxa_sums(ds_nrhiz_top.ps), decreasing = TRUE))
ds_nrhiz_top.name <- c("Other", ds_nrhiz_top.name)
ds_nrhiz_top.colr <- soil.colr[unique(ds_nrhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
ds_nrhiz_top.df <- psmelt(ds_nrhiz_top.ps)
ds_nrhiz_top.df$ASVs <- factor(ds_nrhiz_top.df$ASV, levels = unique(ds_nrhiz_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
ds_crhiz_top.ps <- aggregate_top_taxa2(ds_crhiz.ps, 19, "ASV")
ds_crhiz_top.name <- names(sort(taxa_sums(ds_crhiz_top.ps), decreasing = TRUE))
ds_crhiz_top.name <- c("Other", ds_crhiz_top.name)
ds_crhiz_top.colr <- soil.colr[unique(ds_crhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
ds_crhiz_top.df <- psmelt(ds_crhiz_top.ps)
ds_crhiz_top.df$ASVs <- factor(ds_crhiz_top.df$ASV, levels = unique(ds_crhiz_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
ds_nroot_top.ps <- aggregate_top_taxa2(ds_nroot.ps, 19, "ASV")
ds_nroot_top.name <- names(sort(taxa_sums(ds_nroot_top.ps), decreasing = TRUE))
ds_nroot_top.name <- c("Other", ds_nroot_top.name)
ds_nroot_top.colr <- root.colr[unique(ds_nroot_top.name),]

# Create a data.frame with all the relevant information for plotting #
ds_nroot_top.df <- psmelt(ds_nroot_top.ps)
ds_nroot_top.df$ASVs <- factor(ds_nroot_top.df$ASV, levels = unique(ds_nroot_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
ds_croot_top.ps <- aggregate_top_taxa2(ds_croot.ps, 19, "ASV")
ds_croot_top.name <- names(sort(taxa_sums(ds_croot_top.ps), decreasing = TRUE))
ds_croot_top.name <- c("Other", ds_croot_top.name)
ds_croot_top.colr <- root.colr[unique(ds_croot_top.name),]

# Create a data.frame with all the relevant information for plotting #
ds_croot_top.df <- psmelt(ds_croot_top.ps)
ds_croot_top.df$ASVs <- factor(ds_croot_top.df$ASV, levels = unique(ds_croot_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
hp_nbulk_top.ps <- aggregate_top_taxa2(hp_nbulk.ps, 19, "ASV")
hp_nbulk_top.name <- names(sort(taxa_sums(hp_nbulk_top.ps), decreasing = TRUE))
hp_nbulk_top.colr <- soil.colr[hp_nbulk_top.name,]

# Create a data.frame with all the relevant information for plotting #
hp_nbulk_top.df <- psmelt(hp_nbulk_top.ps)
hp_nbulk_top.df$ASVs <- factor(hp_nbulk_top.df$ASV, levels = hp_nbulk_top.name)
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
hp_nrhiz_top.ps <- aggregate_top_taxa2(hp_nrhiz.ps, 19, "ASV")
hp_nrhiz_top.name <- names(sort(taxa_sums(hp_nrhiz_top.ps), decreasing = TRUE))
hp_nrhiz_top.name <- c("Other", hp_nrhiz_top.name)
hp_nrhiz_top.colr <- soil.colr[unique(hp_nrhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
hp_nrhiz_top.df <- psmelt(hp_nrhiz_top.ps)
hp_nrhiz_top.df$ASVs <- factor(hp_nrhiz_top.df$ASV, levels = unique(hp_nrhiz_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
hp_crhiz_top.ps <- aggregate_top_taxa2(hp_crhiz.ps, 19, "ASV")
hp_crhiz_top.name <- names(sort(taxa_sums(hp_crhiz_top.ps), decreasing = TRUE))
hp_crhiz_top.name <- c("Other", hp_crhiz_top.name)
hp_crhiz_top.colr <- soil.colr[unique(hp_crhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
hp_crhiz_top.df <- psmelt(hp_crhiz_top.ps)
hp_crhiz_top.df$ASVs <- factor(hp_crhiz_top.df$ASV, levels = unique(hp_crhiz_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
hp_nroot_top.ps <- aggregate_top_taxa2(hp_nroot.ps, 19, "ASV")
hp_nroot_top.name <- names(sort(taxa_sums(hp_nroot_top.ps), decreasing = TRUE))
hp_nroot_top.name <- c("Other", hp_nroot_top.name)
hp_nroot_top.colr <- root.colr[unique(hp_nroot_top.name),]

# Create a data.frame with all the relevant information for plotting #
hp_nroot_top.df <- psmelt(hp_nroot_top.ps)
hp_nroot_top.df$ASVs <- factor(hp_nroot_top.df$ASV, levels = unique(hp_nroot_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
hp_croot_top.ps <- aggregate_top_taxa2(hp_croot.ps, 19, "ASV")
hp_croot_top.name <- names(sort(taxa_sums(hp_croot_top.ps), decreasing = TRUE))
hp_croot_top.name <- c("Other", hp_croot_top.name)
hp_croot_top.colr <- root.colr[unique(hp_croot_top.name),]

# Create a data.frame with all the relevant information for plotting #
hp_croot_top.df <- psmelt(hp_croot_top.ps)
hp_croot_top.df$ASVs <- factor(hp_croot_top.df$ASV, levels = unique(hp_croot_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cl_nbulk_top.ps <- aggregate_top_taxa2(cl_nbulk.ps, 19, "ASV")
cl_nbulk_top.name <- names(sort(taxa_sums(cl_nbulk_top.ps), decreasing = TRUE))
cl_nbulk_top.colr <- soil.colr[cl_nbulk_top.name,]

# Create a data.frame with all the relevant information for plotting #
cl_nbulk_top.df <- psmelt(cl_nbulk_top.ps)
cl_nbulk_top.df$ASVs <- factor(cl_nbulk_top.df$ASV, levels = cl_nbulk_top.name)
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cl_crhiz_top.ps <- aggregate_top_taxa2(cl_crhiz.ps, 19, "ASV")
cl_crhiz_top.name <- names(sort(taxa_sums(cl_crhiz_top.ps), decreasing = TRUE))
cl_crhiz_top.name <- c("Other", cl_crhiz_top.name)
cl_crhiz_top.colr <- soil.colr[unique(cl_crhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
cl_crhiz_top.df <- psmelt(cl_crhiz_top.ps)
cl_crhiz_top.df$ASVs <- factor(cl_crhiz_top.df$ASV, levels = unique(cl_crhiz_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cl_nroot_top.ps <- aggregate_top_taxa2(cl_nroot.ps, 19, "ASV")
cl_nroot_top.name <- names(sort(taxa_sums(cl_nroot_top.ps), decreasing = TRUE))
cl_nroot_top.name <- c("Other", cl_nroot_top.name)
cl_nroot_top.colr <- root.colr[unique(cl_nroot_top.name),]

# Create a data.frame with all the relevant information for plotting #
cl_nroot_top.df <- psmelt(cl_nroot_top.ps)
cl_nroot_top.df$ASVs <- factor(cl_nroot_top.df$ASV, levels = unique(cl_nroot_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
cl_croot_top.ps <- aggregate_top_taxa2(cl_croot.ps, 19, "ASV")
cl_croot_top.name <- names(sort(taxa_sums(cl_croot_top.ps), decreasing = TRUE))
cl_croot_top.name <- c("Other", cl_croot_top.name)
cl_croot_top.colr <- root.colr[unique(cl_croot_top.name),]

# Create a data.frame with all the relevant information for plotting #
cl_croot_top.df <- psmelt(cl_croot_top.ps)
cl_croot_top.df$ASVs <- factor(cl_croot_top.df$ASV, levels = unique(cl_croot_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
md_nrhiz_top.ps <- aggregate_top_taxa2(md_nrhiz.ps, 19, "ASV")
md_nrhiz_top.name <- names(sort(taxa_sums(md_nrhiz_top.ps), decreasing = TRUE))
md_nrhiz_top.name <- c("Other", md_nrhiz_top.name)
md_nrhiz_top.colr <- soil.colr[unique(md_nrhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
md_nrhiz_top.df <- psmelt(md_nrhiz_top.ps)
md_nrhiz_top.df$ASVs <- factor(md_nrhiz_top.df$ASV, levels = unique(md_nrhiz_top.name))
...
## Common ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
md_crhiz_top.ps <- aggregate_top_taxa2(md_crhiz.ps, 19, "ASV")
md_crhiz_top.name <- names(sort(taxa_sums(md_crhiz_top.ps), decreasing = TRUE))
md_crhiz_top.name <- c("Other", md_crhiz_top.name)
md_crhiz_top.colr <- soil.colr[unique(md_crhiz_top.name),]

# Create a data.frame with all the relevant information for plotting #
md_crhiz_top.df <- psmelt(md_crhiz_top.ps)
md_crhiz_top.df$ASVs <- factor(md_crhiz_top.df$ASV, levels = unique(md_crhiz_top.name))
...
## Non-PSF ##
# Create a phyloseq with the top 19 asvs and the rest converted to "Other
md_nroot_top.ps <- aggregate_top_taxa2(md_nroot.ps, 19, "ASV")
md_nroot_top.name <- names(sort(taxa_sums(md_nroot_top.ps), decreasing = TRUE))
md_nroot_top.name <- c("Other", md_nroot_top.name)
md_nroot_top.colr <- root.colr[unique(md_nroot_top.name),]

# Create a data.frame with all the relevant information for plotting #
md_nroot_top.df <- psmelt(md_nroot_top.ps)
md_nroot_top.df$ASVs <- factor(md_nroot_top.df$ASV, levels = unique(md_nroot_top.name))
...
md_croot_top.name <- names(sort(taxa_sums(md_croot_top.ps), decreasing = TRUE))
md_croot_top.name <- c("Other", md_croot_top.name)
md_croot_top.colr <- root.colr[unique(md_croot_top.name),]

# Create a data.frame with all the relevant information for plotting #
md_croot_top.df <- psmelt(md_croot_top.ps)
md_croot_top.df$ASVs <- factor(md_croot_top.df$ASV, levels = unique(md_croot_top.name))
...
fb_soil_nod.colr <- soil.colr[fb_nod_soil.name,]
...
fb_soil_nod.plot <- ggplot(fb_soil_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
  scale_fill_manual(name = "Soil ASV", values = fb_soil_nod.colr) +
  scale_y_continuous(sec.axis = dup_axis(name = "<em>S. helvola</em>")) +
  theme_bw() +
  theme(axis.text = element_text(color = "black", size = 18, family = "Liberation Sans"),
        axis.text.x.bottom = element_text(color = "black", size = 24, family = "Liberation Sans", face = 'bold'),
        axis.title = element_text(size = 22, family = "Liberation Sans"),
        ...
        axis.text.y.right = element_blank(),
        axis.ticks.y.right = element_blank(),
        axis.title.y.right = ggtext::element_markdown(size = 24, family = "Liberation Sans", face = 'bold', angle = -90),
        legend.position = 'right')
...
cc_soil_nod.colr <- soil.colr[cc_nod_soil.name,]
...
cc_soil_nod.plot <- ggplot(cc_soil_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
  scale_fill_manual(name = "Soil ASV", values = cc_soil_nod.colr) +
  scale_y_continuous(sec.axis = dup_axis(name = "<em>C. fasciculata</em>")) +
  theme_bw() +
  theme(axis.text = element_text(color = "black", size = 18, family = "Liberation Sans"),
        axis.text.x.bottom = element_text(color = "black", size = 24, family = "Liberation Sans", face = 'bold'),
        axis.title = element_text(size = 22, family = "Liberation Sans"),
...
ds_soil_nod.colr <- soil.colr[ds_nod_soil.name,]
...
ds_soil_nod.plot <- ggplot(ds_soil_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
  scale_fill_manual(name = "Soil ASV", values = ds_soil_nod.colr) +
  scale_y_continuous(sec.axis = dup_axis(name = "<em>D. canadense</em>")) +
  theme_bw() +
  theme(axis.text = element_text(color = "black", size = 18, family = "Liberation Sans"),
        axis.text.x.bottom = element_text(color = "black", size = 24, family = "Liberation Sans", face = 'bold'),
        axis.title = element_text(size = 22, family = "Liberation Sans"),
...
hp_soil_nod.colr <- soil.colr[hp_nod_soil.name,]
...
cl_soil_nod.colr <- soil.colr[cl_nod_soil.name,]
...
cl_soil_nod.plot <- ggplot(cl_soil_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
  scale_fill_manual(name = "Soil ASV", values = cl_soil_nod.colr) +
  scale_y_continuous(sec.axis = dup_axis(name = "<em>T. repens</em>")) +
  theme_bw() +
  theme(axis.text = element_text(color = "black", size = 18, family = "Liberation Sans"),
        axis.text.x.bottom = element_text(color = "black", size = 24, family = "Liberation Sans", face = 'bold'),
        axis.title = element_text(size = 22, family = "Liberation Sans"),
...
md_soil_nod.colr <- soil.colr[md_nod_soil.name,]
...
md_soil_nod.plot <- ggplot(md_soil_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
  scale_fill_manual(name = "Soil ASV", values = md_soil_nod.colr) +
  scale_y_continuous(sec.axis = dup_axis(name = "<em>M. truncatula</em>")) +
  theme_bw() +
  theme(axis.text = element_text(color = "black", size = 18, family = "Liberation Sans"),
        axis.text.x.bottom = element_text(color = "black", size = 24, family = "Liberation Sans", face = 'bold'),
        axis.title = element_text(size = 22, family = "Liberation Sans"),
...
fb_root_nod.ps <- aggregate_top_taxa2(fb_root_nod_raw.ps, 8, "ASV")
fb_nod_root.name <- names(sort(taxa_sums(fb_root_nod.ps), decreasing = TRUE))
fb_root_nod.colr <- root.colr[fb_nod_root.name,]
...
fb_root_nod.plot <- ggplot(fb_root_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
...
cc_root_nod.colr <- root.colr[cc_nod_root.name,]
...
cc_root_nod.plot <- ggplot(cc_root_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
...
ds_root_nod.colr <- root.colr[ds_nod_root.name,]
...
ds_root_nod.plot <- ggplot(ds_root_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
...
hp_root_nod.colr <- root.colr[hp_nod_root.name,]
...
cl_root_nod.colr <- root.colr[cl_nod_root.name,]
...
cl_root_nod.plot <- ggplot(cl_root_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
...
md_root_nod.colr <- root.colr[md_nod_root.name,]
...
md_root_nod.plot <- ggplot(md_root_nod.df, aes(x = Soils, y = Abundance, fill = ASVs)) +
  geom_bar(stat='identity', position = 'fill') +
  xlab('') +
  ylab('') +
...
npsf_nmds.plot <- ggplot(npsf_nmds.load, aes(NMDS1, NMDS2, color = Plants, shape = Comps)) +
  ...
  theme(legend.position = 'none',
        ...
        axis.title.x = ggtext::element_markdown(family = 'Liberation Sans', face = 'bold', size = 24, color = 'black'),
        axis.title.y = ggtext::element_markdown(family = 'Liberation Sans', face = 'bold', size = 24, color = 'black'))+
  annotate(geom = 'richtext', x = 0.025, y = -0.236,
           ...
           size = 8, family = 'Liberation Sans', fontface = 'bold', fill = NA, label.color = NA) +
  coord_cartesian(ylim = c(-0.235,0.2)) +
  labs(tag = 'B.')
...
wpsf_nmds.plot <- ggplot(wpsf_nmds.load, aes(NMDS1, NMDS2, color = Plants, shape = Comps)) +
  ...
  theme(legend.position = 'right',
        ...
        axis.title.x = ggtext::element_markdown(family = 'Liberation Sans', face = 'bold', size = 24, color = 'black'),
        axis.title.y = ggtext::element_markdown(family = 'Liberation Sans', face = 'bold', size = 24, color = 'black'))+
  annotate(geom = 'richtext', x = 0.02, y = -0.22,
           ...
           size = 8, family = 'Liberation Sans', fontface = 'bold', fill = NA, label.color = NA) +
  coord_cartesian(ylim = c(-0.22,0.15)) +
  labs(tag = "A.")
...
soil_rcur.plot <- ggplot(soil.radf, aes(x = i, y = index, group = ID, color = Group)) +
...
# Plot the Alpha-rarefaction Curve #
root_rcur.plot <- ggplot(root.radf, aes(x = i, y = index, group = ID, color = Group)) +
  geom_smooth(span = 0.7, se = FALSE, method = "loess") +
  theme_bw() +
  xlab('Sequencing Coverage') +
  ylab("Shannon Diversity") +
